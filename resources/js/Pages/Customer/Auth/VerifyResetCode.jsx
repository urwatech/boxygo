...
export default function VerifyResetCode({ email, phone_number }) {
    ...
    const applySequentialDigits = (startIndex, rawDigits) => {
    ...
    const resend = (e) => {
        e.preventDefault();
        post('/resend-reset-code', { data: { email: data.email }, preserveScroll: true, onError: () => {} });
    };
        ...
        <>
            ...
            <div className="text-left md:text-center">
                ...
                <p className="mt-3 text-sm leading-relaxed text-gray-500 md:max-w-[80%] mx-auto">
                    {t('authVerifyResetSubtitleBefore')} {data?.email && <span className="font-semibold text-gray-700">: {data?.email}</span>} {t('authVerifyResetSubtitleAfter')}
                </p>
            ...
            <form onSubmit={onSubmit} className="mt-10 flex flex-col items-center gap-8">
                ...
                <div className="self-start md:self-center text-sm text-gray-500">
                    ...
                    <Link href="/resend-reset-code" onClick={resend} className="font-semibold text-[#3a70ff] hover:text-[#2154ff]">
                        {t('authVerifyResend')}
                    </Link>
                </div>
                <button
                    type="submit"
                    disabled={processing}
                    className="hidden md:block w-full px-16 rounded-full bg-[#338DFF] py-3 text-sm font-semibold text-white uppercase tracking-wider pill-shadow hover:shadow-lg transition-shadow disabled:opacity-60"
                >
                    {t('authVerifyButton')}
                </button>
                <button
                    type="submit"
                    disabled={processing}
                    className="md:hidden absolute bottom-5 w-[90%] mx-auto px-16 rounded-full bg-[#338DFF] py-3 text-sm font-semibold text-white uppercase tracking-wider pill-shadow hover:shadow-lg transition-shadow disabled:opacity-60"
                >
                    {t('commonContinue')}
                </button>
            </form>
    ...
    if (isDesktop) {
        return <AuthShell rightClassName="px-6 sm:px-10 py-10 flex flex-col" rightContent={<VerificationFormContent />} />;
    }
                ...
                <aside className="hidden relative w-full md:w-1/2 bg-gradient-to-br from-[#4f8aff] via-[#3c6dfb] to-[#264be7] p-6 sm:p-8 text-white md:flex flex-col justify-between">
                    <div className="relative flex-1 flex flex-col justify-center md:items-end">
                        <div className="relative mx-auto md:mr-6 w-full max-w-[320px] sm:max-w-[340px] md:max-w-[380px] rounded-[30px] bg-white/12 backdrop-blur-md p-6 sm:p-8 testimonial-card min-h-[420px] md:min-h-[500px] flex flex-col justify-between transition-all">
                            <h1 className="mt-10 text-2xl sm:text-3xl font-semibold leading-snug max-w-[260px]">{t('authVerifyResetTestimonial')}</h1>
                            ...
                            <div className="absolute top-10 right-3 sm:right-5 w-16 h-16 rounded-full border border-white/40 bg-white flex items-center justify-center text-lg font-semibold italic text-[#ff8a3d]">
                                <img src="/assets/images/bag.png" alt="icon" />
                            </div>
                            ...
                            <img className="absolute bottom-0 right-0 sm:-right-6 md:-right-16" src="/assets/images/new_password.png" alt="password reset" />
                        </div>
                    </div>
